      ...
      .map((msg) => {
        ...
        try {
          ...
          return JSON.stringify(
            msg,
            (_, value) => (typeof value === "bigint" ? value.toString() + "n" : value) // Example: handle BigInt
          )
        } catch {
...
export async function getSelectionPosition(
  ...
  const content = decoder.decode(await Deno.readFile(filePath)).replace(/\r\n/g, "\n")
  ...
  const scan = (startLineIndex: number, direction: "up" | "down") => {
    ...
    for (
      ...
      direction === "up" ? currentLineIndex >= 0 : currentLineIndex <= fileLines.length - selLen;
  ...
  const startColumn = findColumn(fileLines[actualStartLineIndex], selectionLines[0]) // For endColumn, find the column of the last line of selection within the corresponding file line
  ...
  if (startColumn === -1 || endColumnOfLastSelectionLineStart === -1) {
    ...
    Logger.warn("Could not determine column for selection, though lines matched.")
    return null
  } // Calculate the end column based on the start of the last selection line and its length

  const endColumn = endColumnOfLastSelectionLineStart + selectionLines[selectionLines.length - 1].length - 1
 ...
 */
export async function runCommand(cmd: string[], options?: { ignoreExitCode?: boolean }): Promise<CommandOutput> {
...
export function normalizeRemoteUrl(remoteUrlStr: string): string {
  ...
  if (tempUrl.startsWith("git@")) {
    ...
    try {
      ...
      if (match && match[1] && match[2]) {
        ...
        Logger.warn(`Used regex fallback for SSH URL normalization: ${tempUrl}`)
      } else {
  ...
  try {
    ...
    if (finalParsedUrl.protocol !== "https:") {
      throw new Error(`URL scheme is '${finalParsedUrl.protocol}' not 'https:'.`)
    } // Remove trailing slash from pathname if it's not the root itself
    if (finalParsedUrl.pathname !== "/" && finalParsedUrl.pathname.endsWith("/")) {
    ...
    throw new Error(`Failed to parse normalized URL '${tempUrl}' (from '${remoteUrlStr}'). Error: ${originalError}`)
  }
}
...
export async function getDefaultBranchName(): Promise<string> {
  try {
    ...
    if (success && stdout && stdout !== "origin/HEAD") {
      ...
      Logger.warn("Could not determine default branch directly from origin/HEAD. Trying 'symbolic-ref'.")
      const symbolicRefResult = await runCommand(["git", "symbolic-ref", "refs/remotes/origin/HEAD"], {
        ignoreExitCode: true,
      })
      ...
      Logger.warn("Still could not determine default branch. Falling back to 'main'. This might be incorrect.")
      return "main"
    }
  } catch (error) {
...
export async function main() {
  ...
  if (parsedLineEnd != null && parsedLineStart == null) {
    Logger.error("--line-end can only be used if --line-start is also provided.")
    Deno.exit(1)
  }
  if (parsedLineStart != null && parsedLineEnd != null && parsedLineEnd < parsedLineStart) {
    Logger.error(`--line-end (${parsedLineEnd}) must be >= --line-start (${parsedLineStart}).`)
    Deno.exit(1)
  ...
  if (selectionTextFromArg != null) {
    ...
    try {
      ...
      const selectionRange = await getSelectionPosition(filePath, selectionTextFromArg, parsedLineStart)
      ...
      Logger.error(`Error reading or processing file for selection: ${filePath}`)
    ...
    if (finalLineStart != null && filePath === undefined) {
      Logger.error("--line-start can only be used if a <file_path> is also provided.")
      Deno.exit(1)
    }
  ...
  try {
    const originResult = await runCommand(["git", "remote", "get-url", "origin"], { ignoreExitCode: true })
    if (originResult.success && originResult.stdout) {
      remoteUrlRaw = originResult.stdout
    } else {
      Logger.warn("Remote 'origin' not found. Trying first available remote...")
      const remotesResult = await runCommand(["git", "remote"], { ignoreExitCode: true })
      ...
      const firstRemoteUrlResult = await runCommand(["git", "remote", "get-url", firstRemoteName])
      remoteUrlRaw = firstRemoteUrlResult.stdout
    }
  } catch (error) {
    Logger.error("Failed to get remote URL:", error instanceof Error ? error.message : String(error))
    Deno.exit(1)
  ...
  if (useDefaultBranch) {
    ...
    if (detectedBranch === "HEAD") {
      ...
      ;({ stdout: detectedBranch } = await runCommand(["git", "rev-parse", "HEAD"])) // Get full SHA
    }
  ...
  if (filePath) {
    ...
    if (finalLineStart != null) {
      ...
      if (providerHostname.includes("github.com") || providerHostname.includes("gitlab.com")) {
        fragment = finalLineEnd != null ? `#L${lineStartStr}-L${finalLineEnd}` : `#L${lineStartStr}`
      } else if (providerHostname.includes("bitbucket.org")) {
        fragment = finalLineEnd != null ? `#lines-${lineStartStr}:${finalLineEnd}` : `#lines-${lineStartStr}`
      }
    ...
    if (providerHostname.includes("github.com")) {
      ...
      Logger.error(`Unsupported provider at host '${providerHostname}'. Cannot construct file URL.`)
      Deno.exit(1)
    }
  ...
  if (openCmdName) {
    ...
    if (code !== 0) {
      ...
      Logger.error(`Error opening URL via ${openCmdName}: ${errorMsg || `(code: ${code})`}`) // Retry for Windows "start" command with 'cmd /c start "" "URL"'

      if (Deno.build.os === "windows" && openCmdName === "start" && argsForOpen.length === 2) {
        Logger.info('Retrying with \'cmd /c start "" "URL"\' for Windows...')
        const escapedUrl = finalUrlToOpen.replace(/&/g, "^&").replace(/%/g, "%%") // Escape special cmd characters
        const cmdStart = new Deno.Command("cmd", {
          args: ["/c", "start", '""', `"${escapedUrl}"`], // Quotes for safety
          stdout: "piped",
          stderr: "piped",
        })
        const { code: cmdCode, stderr: cmdStderrBytes } = await cmdStart.output()
        if (cmdCode !== 0) {
          Logger.error(
...
if (import.meta.main) {
  main().catch((err) => {
    ...
    Logger.error("Unhandled error in main execution:", err instanceof Error ? err.message : String(err))
