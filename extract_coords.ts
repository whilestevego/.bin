...
async function extractWithExiftool(filePaths: string[]) {
  const cmd = new Deno.Command("exiftool", {
    args: ["-json", "-n", "-GPSLatitude", "-GPSLongitude", "-CreateDate", ...filePaths],
  })
...
async function collectFiles(pathOrGlob: string): Promise<string[]> {
  ...
  try {
    ...
    if (info.isDirectory) {
      for await (const entry of expandGlob(`${pathOrGlob}/**/*.MOV`, { caseInsensitive: true })) {
...
if (outputCSV) {
  ...
  for (const r of results) {
    console.log(`${r.file},${r.latitude ?? ""},${r.longitude ?? ""},${r.creationDate ?? ""}`)
  }
} else {
